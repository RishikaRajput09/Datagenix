...
const PRODUCTS = [
  {
    ...
    features: [
      { icon: "◈", label: "Universal Connectivity", desc: "Plug-and-play connectors for 200+ device types" },
      { icon: "⬡", label: "Real-Time Telemetry", desc: "Sub-10ms latency across every node in your mesh" },
      { icon: "◉", label: "Zero-Code Integration", desc: "Works with legacy ERP and SCADA out of the box" },
      { icon: "◆", label: "Self-Healing Topology", desc: "Automatic failover — the mesh never sleeps" },
      { icon: "▣", label: "Flex Deployment", desc: "On-premise, cloud, or hybrid — your call" },
    ],
    stats: [
  ...
  },
  {
    ...
    features: [
      { icon: "◈", label: "Autonomous Orchestration", desc: "Workflows run themselves across every department" },
      { icon: "⬡", label: "Predictive Maintenance", desc: "Models trained on your sensor data, not generic sets" },
      { icon: "◉", label: "NL Querying", desc: "Ask your dashboards questions in plain language" },
      { icon: "◆", label: "Explainable AI", desc: "Audit-ready logs for every automated decision" },
      { icon: "▣", label: "Adaptive Learning", desc: "Gets smarter from every human override" },
    ],
    stats: [
  ...
  },
  {
    ...
    features: [
      { icon: "◈", label: "Role-Specific Tracks", desc: "Ops, finance, engineering, and leadership paths" },
      { icon: "⬡", label: "Live-Data Labs", desc: "Hands-on sessions inside your own environment" },
      { icon: "◉", label: "Expert Mentorship", desc: "Practitioners, not just instructors" },
      { icon: "◆", label: "Cohort Accountability", desc: "Peer learning pods with shared milestones" },
      { icon: "▣", label: "Industry Certs", desc: "Certification paths aligned to market standards" },
    ],
    stats: [
  ...
  },
  {
    ...
    features: [
      { icon: "◈", label: "AI Narratives", desc: "Executive summaries auto-written from raw metrics" },
      { icon: "⬡", label: "Scenario Modeling", desc: "Forecast with confidence intervals, not guesswork" },
      { icon: "◉", label: "Anomaly Detection", desc: "Root-cause attribution at a single click" },
      { icon: "◆", label: "Multi-Source Blend", desc: "ERP, CRM, IoT, APIs — all in one surface" },
      { icon: "▣", label: "White-Label Reports", desc: "Client-ready output, branded to your standards" },
    ],
    stats: [
      ...
      >
        ...
        {showChart && (
          <div style={{
            ...
            border: "1px solid rgba(33,198,207,0.18)", borderRadius: "12px",
            padding: "12px",
            transform: hovered ? "translateY(-4px)" : "translateY(0)",
            transition: "transform 0.4s ease",
          }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "8px" }}>
              <span style={{ fontFamily: "'Syne',sans-serif", fontSize: "0.72rem", fontWeight: 700, color: "#fff" }}>AI Adoption</span>
              <span style={{ fontFamily: "'DM Sans',sans-serif", fontSize: "0.55rem", letterSpacing: "0.18em", textTransform: "uppercase", color: "#21C6CF" }}>Live</span>
            </div>
            <div style={{ display: "flex", alignItems: "flex-end", gap: "2px", height: "32px" }}>
              {bars.map((h, i) => (
                <div key={i} style={{ flex: 1, borderRadius: "2px", height: `${h}%`, background: i === 5 ? "#21C6CF" : "rgba(33,198,207,0.22)", transition: "height 0.4s ease" }} />
              ))}
            </div>
            <div style={{ display: "flex", justifyContent: "space-between", marginTop: "5px" }}>
              {days.map((d, i) => (
                <span key={i} style={{ fontFamily: "'DM Sans',sans-serif", fontSize: "0.5rem", color: "rgba(255,255,255,0.3)" }}>{d}</span>
              ))}
            </div>
          </div>
        )}
      ...
      <div style={{ display: "flex", gap: "8px", marginTop: "10px" }}>
        {images.map((src, i) => (
          ...
          >
            ...
            />
            {i === active && (
              <div style={{ position: "absolute", inset: 0, background: "rgba(33,198,207,0.08)" }} />
            )}
          </button>
        ))}
      </div>
  ...
  return (
    ...
    >
      ...
      <div style={{
        ...
        display: "flex", alignItems: "center", justifyContent: "center",
        color: "#21C6CF", fontSize: "0.8rem",
        transition: "border-color 0.25s",
      }}>
        {icon}
      </div>
      <div>
        <div style={{ fontFamily: "'Syne',sans-serif", fontSize: "0.82rem", fontWeight: 700, color: hov ? "#fff" : "rgba(255,255,255,0.85)", marginBottom: "2px", transition: "color 0.2s" }}>
          {label}
        </div>
        <div style={{ fontFamily: "'DM Sans',sans-serif", fontSize: "0.75rem", lineHeight: 1.5, color: "rgba(255,255,255,0.38)" }}>
          {desc}
        </div>
      </div>
    </li>
  ...
  return (
    <div
      ...
      style={{
        ...
        padding: "14px 10px", borderRadius: "12px", flex: 1,
        background: hov ? "rgba(33,198,207,0.06)" : "#0a0a0a",
        border: `1px solid ${hov ? "rgba(33,198,207,0.25)" : "rgba(33,198,207,0.08)"}`,
        transition: "all 0.3s ease", cursor: "default",
      }}
    >
      <span style={{ fontFamily: "'Syne',sans-serif", fontSize: "1.5rem", fontWeight: 800, color: "#21C6CF", lineHeight: 1, marginBottom: "4px" }}>{value}</span>
      <span style={{ fontFamily: "'DM Sans',sans-serif", fontSize: "0.65rem", letterSpacing: "0.1em", textTransform: "uppercase", color: "rgba(255,255,255,0.35)", textAlign: "center" }}>{label}</span>
    </div>
  );
}
  ...
  return (
    <button
      ...
      style={{
        ...
        color: "#050505", background: hov ? "#fff" : "#21C6CF",
        padding: "11px 22px", borderRadius: "8px", border: "none", cursor: "pointer",
        boxShadow: hov ? "0 0 36px rgba(33,198,207,0.55)" : "0 0 20px rgba(33,198,207,0.28)",
        transition: "all 0.3s ease",
      }}
    >
      {label}
    ...
    <div ref={sectionRef}>
      ...
      <section style={{ padding: "clamp(50px,4vw,100px) 0", position: "relative", overflow: "hidden" }}>
        ...
        <div style={{
          ...
          opacity: inView ? 1 : 0, transition: "opacity 1.2s ease", pointerEvents: "none",
        }} />
        ...
        <div style={{ position: "relative", zIndex: 1, maxWidth: "1200px", margin: "0 auto", padding: "0 clamp(20px,4vw,60px)" }}>
          <div
            style={{ display: "flex", flexDirection: "column", gap: "clamp(28px,4vw,56px)", alignItems: "stretch" }}
          ...
          >
            ...
            <div style={{ ...slideContent, minWidth: 0, display: "flex", flexDirection: "column", gap: "0" }} className="pcol">
              ...
              {/* Title */}
              <h2 style={{ fontFamily: "'Syne',sans-serif", fontSize: "clamp(2rem,4.5vw,2.5 rem)", fontWeight: 800, color: "#fff", lineHeight: 1.05, letterSpacing: "-0.02em", margin: "0 0 8px" }}>
                {product.title}
              </h2>
              ...
              <ul style={{ listStyle: "none", padding: 0, margin: "0 0 16px", display: "flex", flexDirection: "column", gap: "2px" }}>
                {product.features.map((f, i) => (
                  <FeatureRow key={i} icon={f.icon} label={f.label} desc={f.desc} delay={i * 60} />
                ))}
              </ul>
      ...
      <main className="bg-linear-to-r from-[#140c30] via-[#153D4C] to-[#16A085]" style={{minHeight: "100vh", position: "relative", overflowX: "hidden" }}>
        ...
        <header style={{ position: "relative", zIndex: 1, maxWidth: "1200px", margin: "0 auto", padding: "clamp(60px,10vw,100px) clamp(20px,4vw,60px) clamp(4px,1.5vw,8px)" }}>
          ...
          >
            ...
            <span className="text-[#21C6CF]">AI transformation</span>
          </h1>
          <p style={{
            fontFamily: "'DM Sans',sans-serif", fontSize: "0.95rem", lineHeight: 1.75,
            color: "rgba(255,255,255,0.38)", maxWidth: "500px",
            margin: "0 0 28px",
          ...
          {/* Nav pills */}
          <div style={{ display: "flex", flexWrap: "wrap", gap: "4px", animation: "heroFadeUp 0.8s ease 0.3s both" }}>
            {PRODUCTS.map(p => <NavPill key={p.id} href={`#product-${p.id}`} label={p.title} />)}
          </div>
        </header>
