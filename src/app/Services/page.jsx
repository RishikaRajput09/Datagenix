...
function ConsultingVisual() {
  ...
  const edges = [
    ...
    [1, 2], [1, 3], [1, 7], [2, 4], [2, 8], [3, 5], [3, 9], [4, 6], [5, 6], [7, 2], [8, 4],
  ];
  return (
    <svg viewBox="0 0 480 340" className="w-full h-full" xmlns="http://www.w3.org/2000/svg">
        ...
        return (
          <g key={i}>
            <line x1={x1} y1={y1} x2={x2} y2={y2}
              stroke="rgba(33,198,207,0.12)" strokeWidth="1.5" />
            {/* Travelling pulse */}
            <line x1={x1} y1={y1} x2={x2} y2={y2}
              stroke="#21C6CF" strokeWidth="2" strokeOpacity="0.7"
              strokeDasharray={`6 ${len}`} strokeDashoffset="0" filter="url(#cGlow)">
      ...
      {nodes.map(([x, y, r, label], i) => (
        <g key={i} filter={i === 0 ? "url(#cGlow2)" : undefined}>
          {/* Outer pulse ring */}
          <circle cx={x} cy={y} r={r + 8} fill="none" stroke="#21C6CF" strokeOpacity="0.0">
            <animate attributeName="r" from={r + 4} to={r + 22} dur={`${2 + i * 0.4}s`} repeatCount="indefinite" begin={`${i * 0.3}s`} />
            <animate attributeName="stroke-opacity" from="0.4" to="0" dur={`${2 + i * 0.4}s`} repeatCount="indefinite" begin={`${i * 0.3}s`} />
          </circle>
          {/* Main body */}
          <circle cx={x} cy={y} r={r}
            ...
            strokeWidth={i === 0 ? 2 : 1} />
          {/* Inner bright dot */}
          <circle cx={x} cy={y} r={i === 0 ? 8 : 3} fill="#21C6CF" opacity={i === 0 ? "1" : "0.9"} />
          {label && (
            <text x={x} y={y + r + 13} textAnchor="middle" fontSize="8"
              fill="rgba(33,198,207,0.8)" fontFamily="monospace" letterSpacing="1">{label}</text>
          )}
        </g>
      ))}

      {/* Corner label */}
      <text x="24" y="320" fontSize="9" fill="rgba(33,198,207,0.3)" fontFamily="monospace" letterSpacing="3">NEURAL STRATEGY MESH</text>
    </svg>
  );
}
...
function DataVisual() {
  ...
  return (
    <svg viewBox="0 0 480 340" className="w-full h-full" xmlns="http://www.w3.org/2000/svg">
      ...
      {/* ── Terminal panel ── */}
      <rect x="20" y="20" width="240" height="200" rx="6"
        fill="rgba(10,10,10,0.9)" stroke="rgba(33,198,207,0.35)" strokeWidth="1" />
      {/* Title bar */}
      <rect x="20" y="20" width="240" height="24" rx="6"
      ...
      <text x="140" y="36" textAnchor="middle" fontSize="8"
        fill="rgba(33,198,207,0.7)" fontFamily="monospace">pipeline.stream — bash</text>

      {/* Scrolling lines */}
      <g clipPath="url(#termClip)">
        {streamLines.map((line, i) => (
          <text key={i} x="28" y={62 + i * 22} fontSize="8.2"
            fill={line.includes("[OK]") ? "rgba(33,198,207,0.85)" : "rgba(255,255,255,0.45)"}
      ...
      {/* ── Bar chart ── */}
      <rect x="275" y="20" width="185" height="200" rx="6"
        fill="rgba(10,10,10,0.8)" stroke="rgba(33,198,207,0.2)" strokeWidth="1" />
      <text x="367" y="38" textAnchor="middle" fontSize="8"
        fill="rgba(33,198,207,0.6)" fontFamily="monospace" letterSpacing="2">THROUGHPUT / s</text>
      {/* Grid lines */}
      {[0, 1, 2, 3].map(i => (
        <line key={i} x1="285" y1={165 - i * 35} x2="450" y2={165 - i * 35}
          stroke="rgba(33,198,207,0.07)" strokeWidth="1" />
      ))}
      {barHeights.map((h, i) => (
        <g key={i}>
          <rect x={288 + i * 16} y={165 - h} width="11" height={h} rx="2"
            fill="url(#barGrad)" filter="url(#dGlow)">
            <animate attributeName="height" from="0" to={h} dur={`${0.6 + i * 0.08}s`} fill="freeze" begin={`${i * 0.05}s`} />
            <animate attributeName="y" from="165" to={165 - h} dur={`${0.6 + i * 0.08}s`} fill="freeze" begin={`${i * 0.05}s`} />
          </rect>
          {/* Top glow cap */}
          <rect x={288 + i * 16} y={165 - h} width="11" height="3" rx="1" fill="#21C6CF" opacity="0.9" filter="url(#dGlow)">
            <animate attributeName="y" from="165" to={165 - h} dur={`${0.6 + i * 0.08}s`} fill="freeze" begin={`${i * 0.05}s`} />
          </rect>
        </g>
      ))}
      ...
      {[60, 100, 140, 180].map((y, i) => (
        <g key={i}>
          <line x1="260" y1={y} x2="275" y2={y} stroke="rgba(33,198,207,0.25)" strokeWidth="1" strokeDasharray="3 2">
            <animate attributeName="stroke-opacity" from="0.5" to="0.1" dur={`${0.8 + i * 0.2}s`} repeatCount="indefinite" direction="alternate" />
          </line>
        </g>
      ))}

      {/* ── Bottom stats bar ── */}
      {[["RECORDS/S", "4.2M", 60], ["LATENCY", "12ms", 200], ["UPTIME", "99.98%", 340]].map(([label, val, x], i) => (
        <g key={i}>
          <rect x={x} y="238" width="100" height="48" rx="4"
            fill="rgba(33,198,207,0.06)" stroke="rgba(33,198,207,0.2)" strokeWidth="1" />
          <text x={x + 50} y="256" textAnchor="middle" fontSize="7.5"
            fill="rgba(33,198,207,0.5)" fontFamily="monospace" letterSpacing="2">{label}</text>
          <text x={x + 50} y="275" textAnchor="middle" fontSize="16"
            fill="#21C6CF" fontFamily="monospace" fontWeight="bold"
            filter="url(#dGlow)">{val}</text>
        </g>
      ))}
    </svg>
  );
...
function AutomationVisual() {
  ...
  return (
    <svg viewBox="0 0 480 340" className="w-full h-full" xmlns="http://www.w3.org/2000/svg">
      ...
      {/* Outer orbit ring */}
      <circle cx={cx} cy={cy} r={r1} fill="none" stroke="rgba(33,198,207,0.12)" strokeWidth="1" />
      {/* Tick marks on orbit */}
      {Array.from({ length: 36 }).map((_, i) => {
        const a = (i / 36) * Math.PI * 2;
        const inner = r1 - 4, outer = r1 + 4;
        return <line key={i}
          x1={cx + Math.cos(a) * inner} y1={cy + Math.sin(a) * inner}
          x2={cx + Math.cos(a) * outer} y2={cy + Math.sin(a) * outer}
          stroke="rgba(33,198,207,0.2)" strokeWidth="1" />;
      })}

      {/* Spinning dashed orbit */}
      <circle cx={cx} cy={cy} r={r1} fill="none"
        stroke="rgba(33,198,207,0.35)" strokeWidth="1.5" strokeDasharray="8 6">
      ...
      <circle cx={cx} cy={cy} r={r2} fill="none"
        stroke="rgba(33,198,207,0.18)" strokeWidth="1" strokeDasharray="4 5">
        <animateTransform attributeName="transform" type="rotate"
      ...
      {steps.map((step, i) => {
        ...
        return (
          <g key={i} filter={isActive ? "url(#aGlow)" : undefined}>
            {/* Node glow aura */}
            <circle cx={nx} cy={ny} r="18" fill="none" stroke="#21C6CF" strokeOpacity="0">
              <animate attributeName="r" from="16" to="30" dur={`${1.5 + i * 0.35}s`} repeatCount="indefinite" begin={`${i * 0.5}s`} />
              <animate attributeName="stroke-opacity" from="0.5" to="0" dur={`${1.5 + i * 0.35}s`} repeatCount="indefinite" begin={`${i * 0.5}s`} />
            </circle>
            <circle cx={nx} cy={ny} r="16"
              fill={isActive ? "rgba(33,198,207,0.2)" : "rgba(33,198,207,0.07)"}
              stroke={isActive ? "#21C6CF" : "rgba(33,198,207,0.4)"} strokeWidth={isActive ? 1.5 : 1} />
            <circle cx={nx} cy={ny} r="4"
              fill={isActive ? "#21C6CF" : "rgba(33,198,207,0.5)"} />
            {/* Label */}
            <text x={nx} y={ny + 28} textAnchor="middle" fontSize="7.5"
      ...
      <path id="outerPath" d={`M${cx + r1},${cy} a${r1},${r1} 0 1,1 -0.01,0`}
        fill="none" stroke="none" />

      {/* Core */}
      <circle cx={cx} cy={cy} r="30" fill="rgba(33,198,207,0.15)"
        stroke="#21C6CF" strokeWidth="2" filter="url(#aGlow)" />
      <circle cx={cx} cy={cy} r="18" fill="rgba(33,198,207,0.25)"
        stroke="#21C6CF" strokeWidth="1.5" />
      <text x={cx} y={cy - 4} textAnchor="middle" fontSize="8"
        fill="#21C6CF" fontFamily="monospace" letterSpacing="1">AGENT</text>
      <text x={cx} y={cy + 8} textAnchor="middle" fontSize="8"
        fill="#21C6CF" fontFamily="monospace">CORE</text>

      {/* Right panel — live log */}
      <rect x="320" y="80" width="145" height="165" rx="5"
        fill="rgba(10,10,10,0.85)" stroke="rgba(33,198,207,0.25)" strokeWidth="1" />
      <rect x="320" y="80" width="145" height="20" rx="5"
        fill="rgba(33,198,207,0.1)" />
      <text x="392" y="93" textAnchor="middle" fontSize="7.5"
        fill="rgba(33,198,207,0.7)" fontFamily="monospace" letterSpacing="2">LIVE LOG</text>
      {[
        ...
        <g key={i}>
          <text x="328" y={107 + i * 20} fontSize="7" fill="rgba(33,198,207,0.45)" fontFamily="monospace">{time}</text>
          <text x="328" y={118 + i * 20} fontSize="7.5" fill="rgba(255,255,255,0.65)" fontFamily="monospace"
            filter={i === 4 || i === 5 ? "url(#aSoftGlow)" : undefined}>{msg}
            {i === 5 && <animate attributeName="opacity" values="1;0.4;1" dur="1.5s" repeatCount="indefinite" />}
          </text>
        </g>
      ))}
    </svg>
...
function IoTVisual() {
  ...
  const edges = [[6, 0], [6, 1], [6, 2], [6, 3], [6, 4], [6, 5], [0, 1], [2, 0], [3, 1], [4, 5]];
  return (
    <svg viewBox="0 0 480 340" className="w-full h-full" xmlns="http://www.w3.org/2000/svg">
      <defs>
      ...
      {[120, 90, 60, 30].map((r, i) => (
        <circle key={i} cx="240" cy="170" r={r}
          fill="none" stroke="rgba(33,198,207,0.1)" strokeWidth="1" />
      ))}
      {/* Radar crosshairs */}
      <line x1="240" y1="50" x2="240" y2="290" stroke="rgba(33,198,207,0.07)" strokeWidth="1" />
      <line x1="120" y1="170" x2="360" y2="170" stroke="rgba(33,198,207,0.07)" strokeWidth="1" />

      {/* Radar sweep */}
      <g clipPath="url(#radarClip)">
        <path d="M240,170 L360,170 A120,120 0 0,0 240,50 Z"
          fill="url(#radarGrad)" opacity="0.8">
          <animateTransform attributeName="transform" type="rotate"
            from="0 240 170" to="360 240 170" dur="4s" repeatCount="indefinite" />
        </path>
        {/* Sweep trail */}
        <path d="M240,170 L354,144 A120,120 0 0,0 240,50 Z"
          fill="rgba(33,198,207,0.06)" opacity="0.6">
          <animateTransform attributeName="transform" type="rotate"
            from="0 240 170" to="360 240 170" dur="4s" repeatCount="indefinite" />
        </path>
      </g>
      <circle cx="240" cy="170" r="120" fill="none"
        stroke="rgba(33,198,207,0.25)" strokeWidth="1.5" />

      {/* Mesh edges */}
      {edges.map(([a, b], i) => {
        const [x1, y1] = devices[a]; const [x2, y2] = devices[b];
        return <line key={i} x1={x1} y1={y1} x2={x2} y2={y2}
          stroke="rgba(33,198,207,0.2)" strokeWidth="1" strokeDasharray="5 4">
      ...
      {devices.map(([x, y, label], i) => {
        ...
        return (
          <g key={i} filter={isHub ? "url(#iGlow)" : undefined}>
            {/* Pulse ring */}
            <circle cx={x} cy={y} r={isHub ? 28 : 14} fill="none" stroke="#21C6CF" strokeOpacity="0">
              <animate attributeName="r" from={isHub ? 22 : 10} to={isHub ? 40 : 24}
                dur={`${2 + i * 0.4}s`} repeatCount="indefinite" begin={`${i * 0.3}s`} />
              <animate attributeName="stroke-opacity" from="0.6" to="0"
                dur={`${2 + i * 0.4}s`} repeatCount="indefinite" begin={`${i * 0.3}s`} />
            </circle>
            <circle cx={x} cy={y} r={isHub ? 20 : 12}
              fill={isHub ? "rgba(33,198,207,0.22)" : "rgba(33,198,207,0.08)"}
              stroke={isHub ? "#21C6CF" : "rgba(33,198,207,0.5)"} strokeWidth={isHub ? 2 : 1} />
            <circle cx={x} cy={y} r={isHub ? 8 : 4} fill="#21C6CF" />
            <text x={x} y={y + (isHub ? 34 : 24)} textAnchor="middle" fontSize={isHub ? 8 : 7}
              fill={isHub ? "#21C6CF" : "rgba(33,198,207,0.7)"}
              fontFamily="monospace" letterSpacing="0.5">{label}</text>
          </g>
        );
      })}

      {/* Live status badges */}
      {[["ONLINE", "9/9", 330, 28], ["ALERTS", "0", 420, 28], ["LATENCY", "8ms", 330, 300]].map(([lbl, val, x, y], i) => (
        <g key={i}>
          <rect x={x} y={y} width="65" height="28" rx="3"
            fill="rgba(33,198,207,0.07)" stroke="rgba(33,198,207,0.25)" strokeWidth="1" />
          <text x={x + 32} y={y + 10} textAnchor="middle" fontSize="6.5"
            fill="rgba(33,198,207,0.5)" fontFamily="monospace" letterSpacing="1">{lbl}</text>
          <text x={x + 32} y={y + 22} textAnchor="middle" fontSize="11"
            fill="#21C6CF" fontFamily="monospace" fontWeight="bold">{val}</text>
        </g>
      ))}
    </svg>
  );
  ...
  return (
    <svg viewBox="0 0 480 340" className="w-full h-full" xmlns="http://www.w3.org/2000/svg">
      ...
      {layers.map((lyr, i) => (
        <g key={i}>
          {/* Layer body */}
          <rect x="30" y={lyr.y} width="255" height="38" rx="5"
            fill="url(#layerGrad)"
            stroke={`rgba(33,198,207,${lyr.color})`} strokeWidth={lyr.active ? 1.5 : 1}
            filter={lyr.active ? "url(#clGlow)" : undefined} />
          {/* Left accent bar */}
          <rect x="30" y={lyr.y} width="4" height="38" rx="2"
            fill={`rgba(33,198,207,${lyr.color})`} />
          <text x="50" y={lyr.y + 23} fontSize="9"
            fill={lyr.active ? "rgba(255,255,255,0.85)" : "rgba(255,255,255,0.4)"}
            fontFamily="monospace" letterSpacing="1">{lyr.label}</text>
          {/* Health dot */}
          <circle cx="265" cy={lyr.y + 19} r="5"
            fill={lyr.active ? "#21C6CF" : "rgba(33,198,207,0.3)"}
            filter={lyr.active ? "url(#clGlow)" : undefined}>
            {lyr.active && <animate attributeName="opacity" values="1;0.4;1" dur="2s" repeatCount="indefinite" begin={`${i * 0.4}s`} />}
          </circle>
          {/* Connecting arrow down */}
          {i < layers.length - 1 && (
            <g>
              <line x1="157" y1={lyr.y + 38} x2="157" y2={lyr.y + 52}
                stroke="rgba(33,198,207,0.4)" strokeWidth="1.5" strokeDasharray="3 2">
                <animate attributeName="stroke-dashoffset" from="0" to="-8" dur="0.8s" repeatCount="indefinite" />
              </line>
              <polygon points={`153,${lyr.y + 50} 161,${lyr.y + 50} 157,${lyr.y + 56}`}
      ...
      {/* ── Pod cluster (right) ── */}
      <rect x="305" y="30" width="155" height="240" rx="6"
        fill="rgba(10,10,10,0.8)" stroke="rgba(33,198,207,0.2)" strokeWidth="1" />
      <text x="382" y="50" textAnchor="middle" fontSize="8"
        fill="rgba(33,198,207,0.6)" fontFamily="monospace" letterSpacing="2">POD CLUSTER</text>
      ...
      {pods.map((pod, i) => {
        ...
        return (
          <g key={i}>
            <rect x={px} y={py} width="60" height="50" rx="4"
              fill="rgba(33,198,207,0.06)" stroke="rgba(33,198,207,0.25)" strokeWidth="1" />
            <text x={px + 30} y={py + 14} textAnchor="middle" fontSize="7.5"
              fill="rgba(33,198,207,0.8)" fontFamily="monospace">{pod}</text>
            {/* Utilisation bar */}
            <rect x={px + 6} y={py + 22} width="48" height="6" rx="2"
              fill="rgba(33,198,207,0.1)" />
            <rect x={px + 6} y={py + 22} width={Math.round(48 * util / 100)} height="6" rx="2"
              fill="#21C6CF" opacity="0.8">
              <animate attributeName="width"
                from="0" to={Math.round(48 * util / 100)}
                dur={`${0.8 + i * 0.12}s`} fill="freeze" />
            </rect>
            <text x={px + 30} y={py + 43} textAnchor="middle" fontSize="7"
              fill="rgba(255,255,255,0.4)" fontFamily="monospace">{util}%</text>
          </g>
        );
      })}

      {/* ── Bottom metrics ── */}
      {[["DEPLOY TIME", "43s", 42], ["REPLICAS", "12×", 180], ["ERROR RATE", "0.01%", 318]].map(([lbl, val, x], i) => (
        <g key={i}>
          <rect x={x} y="298" width="100" height="32" rx="3"
            fill="rgba(33,198,207,0.05)" stroke="rgba(33,198,207,0.18)" strokeWidth="1" />
          <text x={x + 50} y="311" textAnchor="middle" fontSize="7"
            fill="rgba(33,198,207,0.45)" fontFamily="monospace" letterSpacing="2">{lbl}</text>
          <text x={x + 50} y="324" textAnchor="middle" fontSize="12"
            fill="#21C6CF" fontFamily="monospace" fontWeight="bold"
            filter="url(#clGlow)">{val}</text>
        </g>
      ))}
    </svg>
  );
    ...
    >
      ...
      <section className="grid-bg relative min-h-screen flex items-center">
        ...
        <div className="max-w-[1200px] px-6 sm:px-10 lg:px-16 relative z-[1] text-left">
          ...
          <Reveal delay={100}>
            ...
            >
              Intelligence{" "}
              <span className="text-[#21C6CF]">engineered</span>
              <br />
              for the{" "}
          ...
          <Reveal delay={200}>
            <p style={{ fontSize: '18px', color: 'rgba(255,255,255,0.55)', lineHeight: 1.7, maxWidth: '720px', margin: '0 0 48px', fontFamily: "'Space Mono',monospace", fontWeight: 400 }}>
              From strategy to deployment — we build AI systems that operate at the intersection of precision, scale, and impact.
            </p>
          </Reveal>
          <Reveal delay={300}>
            <div style={{ display: 'flex', gap: '16px', flexWrap: 'wrap' }}>
              <a href="#services" className="glow-btn">Explore Services</a>
              <a href="#cta" className="glow-btn" style={{ background: 'transparent', color: 'rgba(255,255,255,0.6)', borderColor: 'rgba(255,255,255,0.12)' }}>Contact Us</a>
            </div>
          </Reveal>
        </div>
      </section>
      ...
      <section id="services" style={{ paddingTop: '80px' }}>
        {services.map((svc, i) => {
          ...
          const isEven = i % 2 === 0;
          return (
            <ServiceSection key={svc.id} svc={svc} Visual={Visual} isEven={isEven} index={i} />
          );
        })}
      </section>
  ...
  return (
    ...
    >
      ...
      <div style={{
        ...
        >
          <div className="visual-wrap" style={{ borderRadius: '4px', height: '360px', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '20px', position: 'relative' }}>
            <div style={{ position: 'absolute', inset: 0, background: 'radial-gradient(circle at 50% 50%, rgba(33,198,207,0.04) 0%, transparent 70%)' }} />
            <Visual />
          </div>
        </div>
          ...
          <ul style={{ listStyle: 'none', padding: 0, margin: '0 0 36px', display: 'flex', flexDirection: 'column', gap: '12px' }}>
            {svc.features.map((feat, j) => (
              <li key={j} style={{ display: 'flex', gap: '14px', alignItems: 'flex-start' }}>
                ...
                <span style={{ fontSize: '14px', color: 'rgba(255,255,255,0.7)', fontFamily: "'Space Mono',monospace", lineHeight: 1.6 }}>{feat}</span>
              </li>
            ))}
          </ul>

          <a href="#cta" className="glow-btn">Learn More →</a>
        </div>
      </div>
    </div>
  );
